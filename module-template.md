...
> **注意**：API 参数中的 `String` 类型即为 SafeStr（特殊字符编码为 `%[HEXCODE]`，内置支持），无需再单独标注 `SafeStr`。在文档中直接使用 `String` 描述即可。
...
**广播类型**：`Interrupt`（默认广播类型，外部可通过订阅修改接收类型）
...
> 通常使用同步调用（`-@`）确保操作按序完成；根据需要也可使用异步（`->`）或无应答（`->|`）等其他消息类型。
