...
> 3. 删除不适用的章节。
> 4. 完成后删除本说明块。
> 5. **多模块合并说明**：如果多个 CSM 模块属于同一个 `lvlib` 或 `lvlibp`，可以将它们合并到一个 `.md` 文件中统一描述。此时文件结构如下：
>
>    ```markdown
>    # 此组CSM模块的组名称（lvlib/lvlibp 对应的信息）
>
>    整体介绍（说明该 lvlib/lvlibp 的用途与包含的模块）
>
>    ## 模块1名称
>
>    （当前模板内容）
...
>    ```
>
>    合并后，原模板中每个模块的 `# 一级标题` 降级为 `## 二级标题`，各章节（功能简述、模块信息、API 接口等）相应降级为 `###` 及以下层级。
